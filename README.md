...
本系统面向智能网联汽车（CAV）网关场景，针对 CAN 总线、车载以太网及 V2X 等多协议流量，构建了集异常检测与大语言模型（LLM）语义分析于一体的安全预警平台。系统采用 Profile-First 检测架构（ID 行为 + 时序画像 + 载荷画像为主路径，IForest 作为可选辅助），并通过 LLM 实现异常事件的自动化语义解析与交互式安全问答。
...
针对上述问题，本项目提出了一种**"检测-理解-对话"**三层架构：第一层通过 Profile-First 异常检测（ID 行为 + 时序画像 + 载荷画像为主路径，IForest 作为可选辅助，默认关闭）识别异常信号；第二层利用大语言模型（LLM）对异常事件进行语义分析，包括攻击分类、根因溯源、影响评估及处置建议；第三层通过 Function Calling 机制支持自然语言交互式安全问答，模型可自主调用后端 API 获取实时数据以辅助分析。

**关键设计原则**：
- **显式训练闭环**：检测前必须先调用 `POST /api/anomaly/train` 完成训练，检测接口不再隐式训练
- **Profile-First 主路径**：三类画像检测器（ID/时序/载荷）构成主检测链路，基于训练基线判定异常
- **IForest 辅助定位**：仅在 `enable_iforest_aux=true` 时启用，作为补充信号而非主判依据
- **总线级检测统一**：DoS/Flooding 检测由 TimingProfileDetector 内部的 baseline-aware 逻辑处理，无独立 BusLoadDetector
...
本系统的**Profile-First 画像检测主路径**参考了以下经典工作：
...
### 二、辅助检测：IForest Auxiliary

传统监督学习方法依赖大量标注数据，而车载安全领域中攻击样本通常较为稀缺。Liu et al. 于 2008 年提出的 **Isolation Forest** 算法 [9] 为此类场景提供了有效的无监督检测方案。本项目将其封装为可选的 **IForest Auxiliary**，用于补充画像主链路，而非默认主判。

该算法的核心假设是：异常样本由于其稀疏性，在随机特征分割过程中更易被隔离，因而具有更短的平均路径长度。本系统采用 scikit-learn 的 `IsolationForest` 实现 [10]，特征向量包含：

```
[msg_id_num, payload_len, byte_entropy, protocol_encoded, domain_encoded, delta_t, payload_delta, value_delta]
```
...
本系统采用 Profile-First 异常检测架构，主检测链路由画像模型驱动，Isolation Forest 仅作为辅助模块。
...
- 特征向量：`[msg_id_num, payload_len, byte_entropy, protocol, domain, delta_t, payload_delta, value_delta]`
...
- 若检测器尚未训练，接口返回 `detected=0` 与提示信息，要求先调用 `POST /api/anomaly/train` 执行显式训练流程。
...
可使用脚本 `backend/scripts/evaluate_external_can_datasets.py` 对外部 CAN 数据集进行离线评估，输出包级与事件级指标（Precision / Recall / F1）及检测器分布统计。
