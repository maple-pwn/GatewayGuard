...
- [第六章 Profile-First异常检测引擎](#第六章-profile-first异常检测引擎)
  - [6.1 检测架构设计](#61-检测架构设计)
  - [6.2 画像主路径检测器](#62-画像主路径检测器)
  - [6.3 IForest Auxiliary辅助检测](#63-iforest-auxiliary辅助检测)
...
基于上述背景，本项目设计并实现了GatewayGuard——一个LLM增强的智能网关多协议流量分析与异常预警系统。系统采用 Profile-First 检测架构（ID行为画像 + 时序画像 + 载荷画像为主检测路径，IForest 作为可选辅助模块），并结合 LLM 语义分析能力，实现对CAN总线、车载以太网（SOME/IP）和V2X通信的统一监控与智能预警。
...
2. **Profile-First异常检测**：以ID行为、时序与载荷画像作为主检测链路，并通过可选IForest辅助模块补充异常信号，实现对DoS、Fuzzy、Spoofing等典型攻击的高效检测
...
1. **主辅检测融合架构**：采用Profile-First主检测链路，并引入可选IForest辅助检测与LLM语义分析联动，兼顾检测稳定性、异常覆盖能力和告警可解释性
...
- **第六章 Profile-First异常检测引擎**：详细描述画像主检测链路与可选IForest辅助检测模块的设计与实现
...
| FR-10 | 支持显式训练闭环（需先调用训练接口再执行检测） | 中 |
...
│   ├── services/            # 业务服务层
│   │   ├── anomaly_detector.py  # Profile-First异常检测引擎
...
## 第六章 Profile-First异常检测引擎

### 6.1 检测架构设计

GatewayGuard的异常检测引擎采用 Profile-First 主辅架构：ID行为画像、时序画像、载荷画像构成主检测链路；Isolation Forest 仅作为可选辅助模块，用于补充统计异常信号。系统通过显式训练建立基线后执行检测，兼顾实时性、稳定性与可解释性。

> **[图片位置7]** 建议插入：Profile-First检测架构流程图，展示报文从输入到画像检测、IForest辅助检测、告警输出的完整流程。
...
│  │  ├── fit()                  │    │  ← 显式训练（POST /api/anomaly/train）
│  │  └── predict()              │    │  ← 异常预测
│  └─────────────────────────────┘    │
│                                     │
...
| 维度 | Profile主路径（ID+时序+载荷） | IForest Auxiliary（可选） |
|------|--------------------------------|---------------------------|
| 角色定位 | 默认主检测链路 | 辅助信号补充 |
| 启用方式 | 默认启用 | `enable_iforest_aux=true` 时启用 |
| 可解释性 | 高（画像偏离语义明确） | 中（基于异常分数） |
| 训练要求 | 显式训练后使用 | 显式训练后使用 |
| 适用场景 | 常规实时检测与主判 | 边界样本与统计异常补充判定 |
...
系统采用显式训练闭环，必须先调用训练接口完成训练，再执行检测：

```text
POST /api/anomaly/train
    │
    └──▶ AnomalyDetectorService.train(normal_packets)

POST /api/anomaly/detect
    ├── 未训练：返回 HTTP 428 Precondition Required
    └── 已训练：执行检测链并返回告警
```

系统不在采集流程中自动训练模型，避免隐式状态变化导致检测行为不确定。
...
AnomalyDetectorService作为检测统一入口，协调ID行为、时序画像、Payload画像、IForest辅助检测，以及车辆信号专用检测器（RPM、GEAR、Replay）：
...
class AnomalyDetectorService:
    def __init__(self):
        ...
        self.payload_detector = PayloadProfileDetector()
    ...
    def train(self, normal_packets: List[UnifiedPacket]):
        self.id_detector.fit(normal_packets)
        self.timing_detector.fit(normal_packets)
        self.payload_detector.fit(normal_packets)
        ...
        if settings.detector.enable_replay_detector:
            self.replay_detector.fit(normal_packets)
...
1. **可配置的检测模块**：通过配置项独立控制各检测器：
   - `enable_payload_profile`：Payload画像检测
   - `enable_iforest_aux`：IForest辅助检测
   - `enable_replay_detector`：序列重放检测（需训练）
   - `enable_rpm_detector`：RPM超限/突变检测（规则检测）
   - `enable_gear_detector`：档位状态检测（规则检测）
2. **置信度排序**：所有告警按置信度降序排列，确保最可信的告警优先展示
3. **显式训练约束**：检测前必须先完成显式训练，未训练状态返回HTTP 428并拒绝检测
4. **车辆信号专用检测器**：
   - **RPMDetector**：检测发动机转速超限（>8000 RPM）和突变（>2000 RPM/s），基于规则的弱语义检测
   - **GearDetector**：检测非法档位状态（不在P/R/N/D/1-6范围内），基于规则的弱语义检测
   - **ReplaySequenceDetector**：检测序列重放攻击、计数器回滚、子序列复用，需要训练建立正常序列基线
...
| _detector | AnomalyDetectorService | Profile-First异常检测引擎 |
...
`_persist_and_detect()`方法实现了存库→检测→推送的运行流水线（训练由显式接口独立触发）：
...
从缓冲区取出 detect_batch_size 条报文
    ...
    ├──▶ PacketORM 批量写入 SQLite
    │
    ├──▶ 检测器未训练？ → 返回 428，要求先调用 /api/anomaly/train
    │
    ├──▶ AnomalyDetectorService.detect()
    │       ├── 规则检测
    │       └── ML检测
...
手动触发一次异常检测，对数据库中最近的报文执行画像主路径检测（并按配置可选叠加IForest辅助检测）。
