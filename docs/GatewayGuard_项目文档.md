...
AnomalyDetectorService作为检测统一入口，协调ID行为、时序画像、Payload画像与可选IForest辅助检测：
...
1. **可配置的检测模块**：通过`enable_payload_profile`和`enable_iforest_aux`配置项，可独立控制Payload画像与IForest辅助检测
